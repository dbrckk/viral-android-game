...
class GameSaveStore(context:Context){
 ...
 fun save(state:GameState,nowEpochMillis:Long=System.currentTimeMillis()){
  ...
  val payload=JSONObject().apply{put("schemaVersion",3);put("cash",state.cash);put("gems",state.gems);put("savedAtEpochMillis",nowEpochMillis);put("businesses",levels);put("hiredManagers",managers);put("permanentUpgrades",permanent)}
 ...
 fun restore(nowEpochMillis:Long=System.currentTimeMillis()):RestoreResult{
  ...
  return try{val j=JSONObject(raw);val s=GameState();s.restoreEconomy(j.optDouble("cash",s.cash),j.optInt("gems",s.gems),parseLevels(j.optJSONArray("businesses")),parseIds(j.optJSONArray("hiredManagers")),parseIds(j.optJSONArray("permanentUpgrades")));val saved=j.optLong("savedAtEpochMillis",nowEpochMillis);val seconds=min(((nowEpochMillis-saved).coerceAtLeast(0)/1000),MAX_OFFLINE_SECONDS);val earnings=s.totalIncomePerSecond*seconds;s.addCash(earnings);RestoreResult(s,seconds,earnings)}catch(_:Exception){RestoreResult(GameState(),0,0.0)}
 }
