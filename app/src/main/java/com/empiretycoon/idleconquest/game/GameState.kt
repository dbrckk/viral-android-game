...
class GameState{
 ...
 fun restoreEconomy(cash:Double,gems:Int,levels:Map<String,Int>,hiredManagers:Set<String> = emptySet(),permanentUpgrades:Set<String> = emptySet()){this.cash=cash.coerceAtLeast(0.0);this.gems=gems.coerceAtLeast(0);mutableBusinesses.replaceAll{it.copy(level=levels[it.id]?.coerceAtLeast(1)?:it.level)};hiredManagerIds.clear();hiredManagerIds.addAll(hiredManagers.filter{id->ManagerCatalog.all.any{it.id==id}});purchasedPermanentUpgradeIds.clear();purchasedPermanentUpgradeIds.addAll(permanentUpgrades.filter{id->PermanentUpgradeCatalog.all.any{it.id==id}})}
